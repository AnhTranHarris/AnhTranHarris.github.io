...
(() => {
  ...
  let open=false,closing=false,lastFocus=null,timers=[];
  ...
  css.textContent=`
    ...
    .rv-fragment,.rv-fill,.rv-line,.rv-edge{position:absolute;opacity:0;pointer-events:none}
    .rv-fragment{width:var(--w);height:var(--h);left:var(--sx);top:var(--sy);border-radius:2px;background:linear-gradient(180deg,transparent,var(--c),var(--c2),transparent);filter:drop-shadow(0 0 4px color-mix(in srgb,var(--c) 60%,transparent));animation:rvTransfer var(--dur) cubic-bezier(.22,.65,.28,1) var(--delay) forwards}
    .rv-fill{width:var(--size);height:var(--size);left:var(--x);top:var(--start);border-radius:1px;background:linear-gradient(180deg,var(--c),color-mix(in srgb,var(--c) 35%,transparent));filter:drop-shadow(0 0 3px color-mix(in srgb,var(--c) 55%,transparent));animation:rvFill var(--dur) cubic-bezier(.18,.55,.28,1) var(--delay) forwards}
    ...
    .rv-edge{background:linear-gradient(90deg,#63d5d0,#0f654d,#d8b86a,#fff0b0);filter:drop-shadow(0 0 5px rgba(99,213,208,.55))}.rv-edge.h{height:2px;left:var(--left);top:var(--top);width:var(--width);transform:scaleX(0);transform-origin:left center;animation:rvEdgeH var(--dur) ease var(--delay) forwards}.rv-edge.v{width:2px;left:var(--left);top:var(--top);height:var(--height);transform:scaleY(0);transform-origin:center top;animation:rvEdgeV var(--dur) ease var(--delay) forwards}
    @keyframes rvBackIn{to{opacity:1}}@keyframes rvBackOut{from{opacity:1}to{opacity:0}}
    @keyframes rvTransfer{0%{opacity:0;transform:translate3d(0,0,0) scaleY(.4)}8%{opacity:.95}42%{opacity:.9;transform:translate3d(calc(var(--dx)*.45),calc(var(--dy)*.5),0) scaleY(1.5)}82%{opacity:.82;transform:translate3d(calc(var(--dx)*.9),calc(var(--dy)*.92),0) scaleY(.9)}100%{opacity:0;transform:translate3d(var(--dx),var(--dy),0) scale(.3)}}
    @keyframes rvFill{0%{opacity:0;transform:translateY(-24px) scale(.45)}15%{opacity:.85}76%{opacity:.68}100%{opacity:0;transform:translateY(var(--fall)) scale(.7)}}
  ...
  function buildEffects(sourceRect,targetRect){
    ...
    const colors=[['#63d5d0','#0f654d'],['#1c8a66','#63d5d0'],['#d8b86a','#fff0b0'],['#8fe6d9','#1c8a66']];
    const fragments=mobile?.matches?34:62;
    for(let i=0;i<fragments;i++){
      const [c,c2]=colors[i%colors.length],el=document.createElement('i');el.className='rv-fragment';
      const sx=sourceRect.left+Math.random()*sourceRect.width,sy=sourceRect.top+Math.random()*sourceRect.height;
      const tx=targetRect.left+Math.random()*targetRect.width,ty=targetRect.top+Math.random()*targetRect.height;
      el.style.cssText=`--sx:${sx}px;--sy:${sy}px;--dx:${tx-sx}px;--dy:${ty-sy}px;--w:${1+Math.random()*3}px;--h:${8+Math.random()*30}px;--c:${c};--c2:${c2};--dur:${3000+Math.random()*1700}ms;--delay:${Math.random()*500}ms`;
      fx.appendChild(el);
    }
    const fillCount=mobile?.matches?44:86;
    for(let i=0;i<fillCount;i++){
      const [c]=colors[(i+1)%colors.length],el=document.createElement('i');el.className='rv-fill';
      const x=targetRect.left+6+Math.random()*Math.max(1,targetRect.width-12),start=targetRect.top-30-Math.random()*90;
      const fall=targetRect.height*(.2+Math.random()*.78),size=1+Math.random()*4;
      el.style.cssText=`--x:${x}px;--start:${start}px;--fall:${fall}px;--size:${size}px;--c:${c};--dur:${2200+Math.random()*2100}ms;--delay:${700+Math.random()*900}ms`;
      fx.appendChild(el);
    }
    const loadRows=mobile?.matches?6:9;
    for(let i=0;i<loadRows;i++){
      const el=document.createElement('i');el.className='rv-line';
      const top=targetRect.top+targetRect.height*((i+1)/(loadRows+1));
      el.style.cssText=`--left:${targetRect.left+targetRect.width*.08}px;--top:${top}px;--width:${targetRect.width*.84}px;--dur:${1500+Math.random()*700}ms;--delay:${1300+i*330}ms`;
      fx.appendChild(el);
    }
    const l=targetRect.left,t=targetRect.top,w=targetRect.width,h=targetRect.height,r=l+w,b=t+h;
    [['h',`--left:${l}px;--top:${t}px;--width:${w}px;--dur:2100ms;--delay:500ms`],['v',`--left:${r-2}px;--top:${t}px;--height:${h}px;--dur:2100ms;--delay:1050ms`],['h',`--left:${l}px;--top:${b-2}px;--width:${w}px;--dur:2100ms;--delay:1700ms`],['v',`--left:${l}px;--top:${t}px;--height:${h}px;--dur:2100ms;--delay:2350ms`]].forEach(([d,style])=>{const el=document.createElement('i');el.className=`rv-edge ${d}`;el.style.cssText=style;fx.appendChild(el)});
    fx.classList.add('active');
  }

  function finishOpen(){
    overlay.classList.remove('constructing','revealing');overlay.classList.add('open','frame-ready');fx.classList.remove('active');sourceCard?.classList.remove('resume-source-dissolve');close.focus({preventScroll:true});
  }
  function openResume(e){
    e?.preventDefault();if(open||closing)return;open=true;lastFocus=document.activeElement;clearTimers();scroller.scrollTop=0;
  ...
  function closeResume(){
    if(!open||closing)return;closing=true;clearTimers();overlay.classList.remove('open','revealing');overlay.classList.add('closing','frame-ready');
