...
(() => {
  ...
  const STEP=90,DRAG_GAIN=.41,DIRECTION_LOCK=7,MIN_RELEASE_SPEED=.04,SPRING=7.5,DAMPING=4.8,BUTTON_SPEED=230,MAX_RELEASE_SPEED=300,MIN_SETTLE_TIME=.62,MAX_SETTLE_TIME=1.15;
  const zone=document.createElement('div'); zone.className='carousel-touch-zone'; zone.setAttribute('aria-hidden','true'); Object.assign(zone.style,{position:'absolute',left:'50%',top:'50%',transform:'translate(-50%, -50%)',width:'90%',height:'430px',zIndex:'12',touchAction:'pan-y',background:'transparent',cursor:'grab',userSelect:'none',WebkitUserSelect:'none',pointerEvents:'auto'}); stage.appendChild(zone);
  let angle=0,velocity=0,raf=0,gesture='idle',activePointer=null,startX=0,startY=0,lastX=0,lastT=0,generation=0;
  barrel.style.transition='none';barrel.style.pointerEvents='none';barrel.style.userSelect='none';barrel.style.webkitUserSelect='none';zone.style.height=window.innerWidth<=560?'390px':'430px';
  const normalize=n=>((n%4)+4)%4; function render(){barrel.style.transform=`rotateY(${angle}deg)`;const i=normalize(Math.round(angle/STEP));dots.forEach((d,n)=>d.classList.toggle('on',n===i));}
  function stopAnimation(){if(raf)cancelAnimationFrame(raf);raf=0} function invalidate(){generation++;stopAnimation()}
  function settleToCard(initialVelocity,target){invalidate();const my=generation;let v=initialVelocity,last=performance.now(),started=last,stable=0;const tick=now=>{if(my!==generation)return;const dt=Math.min(.032,Math.max(.008,(now-last)/1000));last=now;const distance=target-angle,abs=Math.abs(distance),elapsed=(now-started)/1000;v+=((distance*SPRING)-(v*DAMPING))*dt;const step=v*dt;if(Math.abs(step)>=abs&&abs>0){angle=target;velocity=0;render();raf=0;return}angle+=step;velocity=v;render();if(Math.abs(target-angle)<.10&&Math.abs(v)<1)stable++;else stable=0;if((stable>=3&&elapsed>=MIN_SETTLE_TIME)||elapsed>=MAX_SETTLE_TIME){angle=target;velocity=0;render();raf=0;return}raf=requestAnimationFrame(tick)};raf=requestAnimationFrame(tick)}
  function begin(e){if(e.pointerType==='mouse'&&e.button!==0)return;if(controls&&controls.contains(e.target))return;invalidate();gesture='pending';activePointer=e.pointerId;startX=lastX=e.clientX;startY=e.clientY;lastT=performance.now();velocity=0;zone.style.cursor='grabbing'}
  function move(e){if(!activePointer||e.pointerId!==activePointer||gesture==='idle')return;const dxT=e.clientX-startX,dyT=e.clientY-startY;if(gesture==='pending'){if(Math.hypot(dxT,dyT)<DIRECTION_LOCK)return;if(Math.abs(dyT)>Math.abs(dxT)){gesture='vertical';activePointer=null;zone.style.cursor='grab';return}gesture='horizontal'}if(gesture!=='horizontal')return;e.preventDefault();const now=performance.now(),dx=e.clientX-lastX,dt=Math.max(8,now-lastT);lastX=e.clientX;lastT=now;angle+=dx*DRAG_GAIN;velocity=(dx*DRAG_GAIN/dt)*1000;render()}
  function end(e){if(!activePointer||e.pointerId!==activePointer)return;const horizontal=gesture==='horizontal',rv=velocity;activePointer=null;gesture='idle';zone.style.cursor='grab';if(!horizontal)return;const dir=Math.sign(rv),current=Math.round(angle/STEP),offset=angle-current*STEP;let targetIndex;if(dir&&Math.abs(rv)>MIN_RELEASE_SPEED*1000){targetIndex=dir>0?Math.ceil(angle/STEP):Math.floor(angle/STEP);if(targetIndex===current)targetIndex+=dir}else{targetIndex=Math.round(angle/STEP);if(Math.abs(offset)>=STEP/2)targetIndex+=Math.sign(offset)}const target=targetIndex*STEP,initial=Math.sign(target-angle)*Math.min(MAX_RELEASE_SPEED,Math.max(45,Math.abs(rv)));settleToCard(initial,target)}
  function cancel(e){if(activePointer!==null&&e?.pointerId!==undefined&&e.pointerId!==activePointer)return;activePointer=null;gesture='idle';zone.style.cursor='grab';velocity=0;render()}
