...
(() => {
  ...
  let edgePhase=0,edgeLastT=0,desktopRenderRaf=0,fallbackEdgeTimer=0,fallbackLastScrollLeft=0;
  ...
  edgeStyle.textContent=`
    ...
    @media(min-width:901px){
      .page::before{border-width:10px;box-shadow:none}
      .desktop-edge-standard .page::before,.desktop-edge-webkit .page::before{padding:10px;border:0;background:conic-gradient(from var(--edge-angle,0deg),transparent 0deg 327deg,rgba(145,98,25,.12) 331deg,rgba(216,184,106,.38) 335deg,rgba(247,204,88,.72) 339deg,rgba(255,232,142,.96) 343deg,rgba(255,247,207,1) 346deg,rgba(255,255,255,1) 349deg,rgba(255,247,207,1) 352deg,rgba(255,232,142,.94) 355deg,rgba(216,184,106,.42) 358deg,transparent 360deg);filter:none;-webkit-mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);-webkit-mask-composite:xor;mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:exclude}
      .desktop-edge-fallback .page::before{border:10px solid rgba(255,220,112,.96);background:none;box-shadow:none;-webkit-mask:none;mask:none}
      .barrel.edge-motion .page{box-shadow:0 28px 65px rgba(0,0,0,.46),0 5px 16px rgba(0,0,0,.25),inset 0 1px rgba(255,255,255,.07),inset 0 0 18px rgba(255,215,112,.38),0 0 24px rgba(255,226,126,.58),0 0 10px rgba(255,249,218,.34)}
      ...
      .desktop-performance-reduced .barrel.edge-motion .page{box-shadow:0 28px 65px rgba(0,0,0,.46),0 5px 16px rgba(0,0,0,.25),inset 0 1px rgba(255,255,255,.07),inset 0 0 12px rgba(255,215,112,.28),0 0 16px rgba(255,226,126,.42),0 0 7px rgba(255,249,218,.24)}
    }
  ...
  function updateEdgeGlow(motionVelocity,now=performance.now()){
    const speed=Math.abs(motionVelocity);if(speed<2){stopEdgeGlow();return}
    const direction=Math.sign(motionVelocity)||1;
    if(desktopQuery?.matches){
      const dt=edgeLastT?clamp((now-edgeLastT)/1000,0,.05):1/60;
      edgeLastT=now;
      const degreesPerSecond=clamp(90+speed*.55,105,260);
      edgePhase=(edgePhase+direction*degreesPerSecond*dt)%360;
    }else{
  ...
  function render(now=performance.now()){if(fallbackMode)return;barrel.style.transform=`rotateY(${angle}deg)`;setDot(Math.round(angle/STEP));updateEdgeGlow(velocity,now)}
  function queueDesktopRender(){if(!desktopQuery?.matches){render();return}if(desktopRenderRaf)return;desktopRenderRaf=requestAnimationFrame(now=>{desktopRenderRaf=0;render(now)})}
  function stopAnimation(){if(raf)cancelAnimationFrame(raf);raf=0;if(desktopRenderRaf)cancelAnimationFrame(desktopRenderRaf);desktopRenderRaf=0}
